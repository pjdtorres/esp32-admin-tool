...
void startClient() {
    if (wifi_staticIP){
        if (!WiFi.config(CharToIP(wifi_ip_static), CharToIP(wifi_gateway), CharToIP(wifi_subnet), CharToIP(wifi_primaryDNS), CharToIP(wifi_secondaryDNS))){
            log("Error: Falhou ao conectar em modo Station"); 
        }
    }
    ...
    delay(50);
    byte b = 0;
    while (WiFi.status() != WL_CONNECTED && b < 2){
        b++;
    ...
    if (WiFi.status() == WL_CONNECTED){
        ...
        log(F("Error: WiFi não conectado"));
        // Parpadeo Random do Led
        blinkRandomSingle(10, 100, WIFILED);
        delay(100);
    } 
}
...
void wifi_setup(){
    WiFi.disconnect();
    // 1) Se esta activo em Modo AP
    log("ap_accessPoint: " + String(ap_accessPoint));
    if (ap_accessPoint){
        startAP();        
        log("Info: WiFI em Modo AP");
    }
...
void wifiLoop(){
    ...
    if (WiFi.status() != WL_CONNECTED && (currentMillis - previousMillisWIFI >= intervalWIFI)){
        ...
        // parpadeo do Tempo ON e Tempo OFF
